// Browser-side chat UI: view state, host-message routing, and feature-specific
// setup/render helpers. Messages ask the extension host to perform actions;
...
// These limits smooth the preview animation only; they do not limit model context.
const MAX_NARRATION_BUFFER_CHARACTERS = 20_000;
const STREAM_FRAME_DELAY_MS = 18;
const STREAM_FINISH_DELAY_MS = 120;
...
// DOM references: composer and session navigation.
...
// Model and embedding profile dialogs.
...
// Settings, permissions and backend controls.
...
// Register settings interactions once when the view loads.
...
// Each host event has one handler. The names are also checked against the shared protocol.
const extensionMessageHandlers = {
  // Status text alone never releases Send; explicit lifecycle events do that.
  status(message) {
  ...
  permissionPolicyUpdated(message) {
  ...
  commandPermissionRequest(message) {
  ...
  providerStreamDelta(message) {
    appendProviderStreamDelta(message.text);
  },

  toolUsageUpdated(message) {
    state.toolUsage = {
...
window.addEventListener('message', ({ data: message }) => {
  if (Object.hasOwn(extensionMessageHandlers, message.command)) {
    extensionMessageHandlers[message.command](message);
  }
});

setupComposerActions();
setupSessionsActions();
setupChatProfilesActions();
setupEmbeddingProfilesActions();
setupSettingsActions();
renderTokenEstimate();
vscode.postMessage({ command: 'setScope', scope: 'project' });
vscode.postMessage({ command: 'ready' });

// Register composer interactions once when the view loads.
...
// Chat messages, safe Markdown and file-change summaries.
...
// Register sessions interactions once when the view loads.
function setupSessionsActions() {
...
// Session list and transcript restoration.
...
// Active request cards and streaming previews.
...
function finalizeProviderNarration() {
  ...
  if (state.streamQueue) {
    state.narrationText = (state.narrationText + state.streamQueue).slice(0, MAX_NARRATION_BUFFER_CHARACTERS);
    state.streamQueue = '';
  }
...
function ensureProviderStreamPump() {
  ...
  state.streamPumpTimer = setTimeout(pumpProviderStream, STREAM_FRAME_DELAY_MS);
}
...
function pumpProviderStream() {
  ...
  if (state.streamQueue) {
    ...
    state.narrationText = (state.narrationText + chunk).slice(0, MAX_NARRATION_BUFFER_CHARACTERS);
...
// Composer availability and profile displays.
...
function renderBackendStatus() {
  ...
  backendSettingsBadgeEl.textContent = BACKEND_STATUS_LABELS[backend.state] || 'Offline';
  restartBackendEl.disabled = state.askPending || !backend.canRestart;
}

// Register chat profiles interactions once when the view loads.
function setupChatProfilesActions() {
  ...
  // Chat-profile validation stays separate from embedding consent rules.
...
// Chat-profile picker and form.
// Only the row layout is shared. Each profile type keeps its own validation and actions.
function createProfilePickerRow({ selected, iconText, name, metadata, baseUrl, showUrl,
  manageLabel, manageTitle, onSelect, onManage }) {
  const option = document.createElement('div');
  option.className = 'model-picker-option';
  option.dataset.selected = String(selected === true);

  const select = document.createElement('button');
  select.type = 'button';
  select.className = 'model-picker-select';
  select.setAttribute('role', 'option');
  select.setAttribute('aria-selected', String(selected === true));
  ...
  const title = document.createElement('span');
  title.className = 'model-picker-name';
  ...
  if (selected) {
    ...
    badge.className = 'model-picker-selected';
  ...
  manage.textContent = manageLabel;
...
function showLlmProfilePicker(profiles) {
  ...
  availableProfiles.forEach((profile) => {
    const row = createProfilePickerRow({
      selected: profile.selected,
      iconText: profile.builtIn ? '✦' : String(profile.name || '?').slice(0, 1),
      name: profile.name,
      metadata: [
      ...
      showUrl: Boolean(profile.baseUrl),
      manageLabel: profile.builtIn ? 'Configure' : 'Edit',
      manageTitle: (profile.builtIn ? 'Configure ' : 'Edit ') + profile.name,
      onSelect: () => {
        closeLlmProfilePicker();
    ...
    llmProfilePickerListEl.appendChild(row);
  });
...
// Register embedding profiles interactions once when the view loads.
function setupEmbeddingProfilesActions() {
  // Embedding-profile actions.
...
// Embedding-profile picker and form.
...
function showEmbeddingProfilePicker(profiles) {
  ...
  availableProfiles.forEach((profile) => {
    const row = createProfilePickerRow({
      selected: profile.selected,
      iconText: profile.provider === 'ollama' ? 'O' : 'AI',
      name: profile.model,
      metadata: [
        profile.providerLabel,
      ...
      showUrl: true,
      manageLabel: 'Edit',
      manageTitle: 'Edit ' + profile.model,
      onSelect: () => {
        closeEmbeddingProfilePicker();
        openSettingsDialog();
    ...
    embeddingProfilePickerListEl.appendChild(row);
  });
