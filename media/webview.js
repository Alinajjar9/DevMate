// The webview handles presentation and user input. The extension owns files, secrets, and model requests.
// Browser state and DOM references shared by the chat controls.
...
const messageHandlers = new Map([
  ['status', handleStatus],
  ['scopeUpdated', handleScopeUpdated],
  ['assistantResponse', handleAssistantResponse],
  ['sessionsUpdated', handleSessionsUpdated],
  ['sessionProjectWarning', handleSessionProjectWarning],
  ['requestCancelling', handleRequestCancelling],
  ['requestCancelled', handleRequestCancelled],
  ['requestFailed', handleRequestFailed],
  ['attachmentsUpdated', handleAttachmentsUpdated],
  ['llmProfilesUpdated', handleLlmProfilesUpdated],
  ['showLlmProfilePicker', handleShowLlmProfilePicker],
  ['showLlmProfileForm', handleShowLlmProfileForm],
  ['llmProfileFormError', handleLlmProfileFormError],
  ...
  ['agentToolActivity', handleAgentToolActivity],
  ['providerStreamReset', handleProviderStreamReset],
...
// Connect controls before requesting the extension's saved state with the ready message.
...
// Register controls once; each handler below performs a single user action.
function setupComposerEvents() {
  ...
  questionEl.addEventListener('keydown', handleQuestionKeydown);
  questionEl.addEventListener('input', renderTokenEstimate);
  continueAgentEl.addEventListener('click', continueAgentRun);
  attachFilesEl.addEventListener('click', pickAttachments);
  attachmentToggleEl.addEventListener('click', toggleAttachments);
}

function setupProfileEvents() {
  llmProfileSelectorEl.addEventListener('click', chooseLlmProfile);
  intelligenceButtonEl.addEventListener('click', toggleIntelligenceMenu);
  document.addEventListener('click', handleOutsideIntelligenceClick);
  document.addEventListener('keydown', handleIntelligenceKeydown);
  document.getElementById('cancelLlmProfilePicker').addEventListener('click', cancelLlmProfilePicker);
  document.getElementById('addLlmProfile').addEventListener('click', addLlmProfile);
  llmProfileProviderEl.addEventListener('change', handleLlmProfileProviderChange);
  document.getElementById('cancelLlmProfile').addEventListener('click', cancelLlmProfileForm);
  deleteLlmProfileEl.addEventListener('click', deleteLlmProfile);
  llmProfileDialogEl.addEventListener('close', handleLlmProfileFormClosed);
  llmProfileFormEl.addEventListener('submit', saveLlmProfile);
}
...
function setupSessionEvents() {
  sessionSelectorEl.addEventListener('click', openSessionHome);
...
function setupBackendEvents() {
  backendStatusEl.addEventListener('click', openBackendLogs);
  restartBackendEl.addEventListener('click', restartBackend);
  openBackendLogsEl.addEventListener('click', openBackendLogs);
}

// Composer actions.
// Start the UI immediately; the extension collects context and owns the actual request.
...
// Resume the extension's saved checkpoint without adding the same question to chat again.
function continueAgentRun() {
...
// Model picker, reasoning menu and profile editor.
function chooseLlmProfile() {
  closeIntelligenceMenu();
  vscode.postMessage({ command: 'chooseLlmProfile' });
}

function toggleIntelligenceMenu(event) {
...
function handleOutsideIntelligenceClick(event) {
  if (!intelligenceControlEl.contains(event.target)) {
    closeIntelligenceMenu();
  }
}

function handleIntelligenceKeydown(event) {
...
function cancelLlmProfilePicker() {
...
function handleLlmProfileProviderChange() {
  renderLlmProfileProvider(true);
}

function cancelLlmProfileForm() {
  closeLlmProfileForm();
}
...
function handleLlmProfileFormClosed() {
...
// Validate the form for quick feedback; the extension validates again and stores the API key.
function saveLlmProfile(event) {
  event.preventDefault();
  setLlmProfileFormError('');
...
// Extension events carry state snapshots and request progress.
function handleExtensionMessage(event) {
  const message = event.data;
  messageHandlers.get(message.command)?.(message);
}
...
function handleScopeUpdated(message) {
  state.scope = message.scope;
  renderScope();
}

// Let queued preview text finish before replacing it with the complete, authoritative answer.
...
function handleSessionProjectWarning(message) {
  sessionProjectWarningEl.textContent = message.message;
  sessionProjectWarningEl.hidden = false;
  showSessionHome(false);
}

function handleRequestCancelling() {
  markWorkingTurnCancelling();
}

// Wait for the extension's cancellation result before allowing another request.
...
function handleAttachmentsUpdated(message) {
...
function handleCloseLlmProfileForm() {
  closeLlmProfileForm();
}

function handlePermissionPolicyUpdated(message) {
...
function handleCommandPermissionRequest(message) {
...
function handleAgentToolActivity(message) {
...
function handleProviderStreamDelta(message) {
  appendProviderStreamDelta(message.text);
}

function handleToolUsageUpdated(message) {
  state.toolUsage = {
...
function handleTokenUsageUpdated(message) {
  state.requestTokenUsage = message.usage;
  renderWorkingTokenUsage();
  renderTokenEstimate();
}

function handleAgentCheckpointUpdated(message) {
...
// Status, usage and settings summaries.
...
// Show a rough input estimate until real usage arrives; character counts are not tokenizer counts.
...
// Chat messages, safe Markdown rendering and file-change links.
...
// Render a small Markdown subset with DOM nodes and textContent; never interpret model text as HTML.
...
// Send supported link clicks to the extension, where workspace and URL checks are enforced.
...
// Recognize likely file references for display; this is not a workspace permission check.
...
// Add lightweight highlighting while keeping every matched token as literal text.
...
function startWorkingTurn() {
  ...
  retry.addEventListener('click', () => {
    ...
    startWorkingTurn();
    renderAskAvailability();
    // Retry the existing turn without recording the user's question twice.
...
// Drop queued callbacks and any delayed final response when a turn stops or restarts.
...
function appendProviderStreamDelta(text) {
  ...
  // Bound the animation buffer; the final response arrives separately and is not cut here.
...
// Keep a short update when the model moves on to a tool call or a new request phase.
...
// Drain the preview queue in small batches, speeding up when text arrives faster than it is shown.
...
// Keep intermediate updates short; the complete final answer is rendered separately.
...
// Freeze a stopped turn as history and offer retry only when the extension says it is possible.
...
// Replace the temporary preview with the complete answer instead of adding a duplicate message.
...
// Disable old approval buttons so they cannot be used after their request has ended.
...
// Permission cards and tool activity.
// Show the proposed file operations and return a decision tied to the extension's request ID.
...
// Show the exact command and working directory before asking the extension to proceed.
...
// Update the existing activity card by ID as a tool moves from running to completed or failed.
...
// Show whether a key exists without loading the stored secret back into the form.
