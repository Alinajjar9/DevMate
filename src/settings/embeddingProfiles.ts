// Define independent embedding profiles with explicit consent for remote code transfer.
// A chat model selection does not silently authorize an embedding provider.
...
export function preferredEmbeddingProfile(
  ...
  const active = typeof activeProfileId === 'string'
    ...
    : undefined;
  // Respect an explicit selection; otherwise prefer a configured local provider before a remote one.
  return active
