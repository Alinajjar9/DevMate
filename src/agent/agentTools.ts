// Keep tool parsing, limits, retry rules, and bounded tool history together.
// These helpers describe allowed calls; ToolExecutor performs them.

import * as path from 'path';
import type { ApiResult } from '../api/types';
import { createHash } from 'crypto';
import { parseRunCommandArguments, MAX_COMMAND_TIMEOUT_SECONDS, MIN_COMMAND_TIMEOUT_SECONDS } from '../workspace/commandTools';
import {
...
export function isRetryableProviderFailure(result: ApiResult<unknown>): boolean {
  // Retry temporary provider HTTP failures, not bad credentials, malformed results, or tool-budget errors.
...
export function validatePythonRequirementsManifest(content: string): string[] {
  // Keep installation reviewable: reject URLs, editable installs, nested files, and custom indexes.
...
// Replace the oldest bulky results first; keep call identities and never mutate the original history.
...
export function agentToolCallSignature(call: AgentToolCall): string {
  // Parse and normalize first so equivalent argument objects have the same duplicate-call identity.
