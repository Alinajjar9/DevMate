// Apply approved workspace file changes through VS Code.
// Inspect before permission and revalidate afterwards because the workspace may change while waiting.
...
export type WorkspaceMutationPermissionFile = {
  path: string;
...
export type WorkspaceMutationExecution = {
  result: string;
...
export type WorkspaceMutationCallbacks = {
  getPermissionPolicy: () => FilePermissionPolicy;
  requestPermission: (
    summary: string,
...
export class WorkspaceMutations {
  constructor(private readonly callbacks: WorkspaceMutationCallbacks) {}
  ...
  async deleteFile(
    ...
    signal?: AbortSignal
  ): Promise<WorkspaceMutationExecution> {
    ...
    const allowed = await this.callbacks.requestPermission(
      `Delete ${relativePath}`,
      [{
        path: relativePath,
        operation: 'delete',
    ...
    // Approval may take time. Recheck trust, symlinks, and content before deleting anything.
    this.assertTrustedFileLifecycle();
  ...
  async relocateFile(
    ...
    this.callbacks.reportStatus('Waiting for permission');
    const allowed = await this.callbacks.requestPermission(
      `${operationLabel} ${relativePath} to ${newRelativePath}`,
      [{
    ...
    // Neither the approved source nor the empty destination may change while the dialog is open.
    this.assertTrustedFileLifecycle();
    await this.assertNoWorkspaceSymlink(folder, relativePath, false);
    await this.assertNoWorkspaceSymlink(folder, newRelativePath, true);
    await this.revalidateLifecycleFile(source);
    ...
    this.callbacks.recordCompletedDiff(
      newRelativePath,
      source.content,
      source.content,
      relativePath
  ...
  async confirmAndApplyFileChanges(
    ...
    // Capture exactly what the user will review. Dirty editor buffers must not be overwritten.
    ...
    const permissionFiles: WorkspaceMutationPermissionFile[] = plannedChanges.map((change) => ({
      path: change.path,
    ...
    // Permission approves these snapshots, not whatever happens to be at the paths later.
    ...
    // Creating parent directories adds another wait; check paths again before building the edit.
    await Promise.all(
    ...
    // Applying an editor edit and saving it to disk are separate operations with separate failures.
  ...
  async assertNoWorkspaceSymlink(
    ...
    // Check every path segment: a normal-looking file can sit below a symlinked directory.
  ...
  private async inspectLifecycleFile(
    folder: vscode.WorkspaceFolder,
    relativePath: string
  ): Promise<InspectedLifecycleFile> {
    await this.assertNoWorkspaceSymlink(folder, relativePath, false);
  ...
  private async revalidateLifecycleFile(source: InspectedLifecycleFile): Promise<void> {
  ...
  private async assertLifecycleDestinationAvailable(
    uri: vscode.Uri,
    relativePath: string
  ): Promise<void> {
    try {
