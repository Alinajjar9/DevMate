...
export function commandSignature(command: ValidatedCommand): string {
  // Remember the exact executable, arguments, and working directory, not just the displayed label.
...
function validateVerificationCommand(executable: string, args: string[]): void {
  // This allowlist covers verification tasks only. Installation has its own consent and validation path.
