// Fit request context into an estimated token budget without silently dropping required input.
// This selects what to send; it does not delete saved chat history or summarize code.
...
// Lower-priority items may be dropped first. Explicit user input is required, not optional.
...
export function createContextBudget(options: ContextBudgetOptions): ContextBudget {
  ...
  // Leave room for the reply, then reserve 10% because character-based token counts are estimates.
...
export function estimateContextTokens(value: string): number {
  // This is a cheap estimate, not a model-specific tokenizer. The safety margin covers some error.
...
export function planContextCandidates<T>(
  ...
  // Keep required items even when they overflow. The caller must report the overflow to the user.
  ...
  // Greedily fit optional items in priority order; an oversized item does not block smaller ones.
...
export function planAskRequestContext(
  ...
  // Search results may be omitted, but selections, attachments, and explicit file scopes may not.
  ...
  // Keep tool-call identities/arguments even if their bulky output is replaced by an omission note.
  ...
  // Send one continuous tail of the conversation, not old replies with missing turns in between.
