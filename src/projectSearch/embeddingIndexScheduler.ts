// Schedule optional background embeddings after the source index is ready.
// Changed work or profiles invalidate the previous job; lexical search remains available.
...
import { readPreferredEmbeddingProfile } from '../settings/embeddingProfiles';
import type { EmbeddingProfileReader } from '../settings/embeddingProfiles';
...
export class EmbeddingIndexScheduler {
  ...
  private async runPending(): Promise<void> {
    ...
    try {
      ...
      // Profile loading is asynchronous; a different workspace or backend may now own the scheduler.
      ...
      const result = await this.api.synchronize(
        access,
        {
          ...
          // Yield between batches so file/profile changes can cancel obsolete work promptly.
          maxBatches: 1
        },
        profile.apiKey,
        controller.signal
      ...
      // Resume only after a successful incomplete batch. Failures wait for a later scheduling event.
