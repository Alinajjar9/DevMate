// Reconcile the current workspace with its stored code index in bounded batches.
// File hashes decide what changed; cancellation stops later batches.
...
export class KnowledgeIndexSynchronizer {
  ...
  private async run(
    ...
    try {
      ...
      // A new chunking algorithm changes boundaries even when the source file itself is unchanged.
      ...
      // Unreadable does not mean deleted. Keep those paths and mark the completed scan as stale.
      ...
      for (const file of [...snapshot.files].sort((left, right) =>
        ...
        // Timestamps alone can miss edits; compare the actual file bytes before reusing stored chunks.
      ...
      // A rename naturally becomes an upsert of the new path plus deletion of the old path.
