// Coalesce file events into index refreshes instead of scanning once per event.
// The coordinator owns cancellation when backend access or the workspace changes.
...
export class WorkspaceIndexCoordinator implements vscode.Disposable {
  ...
  private requestSynchronization(immediately: boolean): void {
    // Many save events can describe one user action. Record pending work rather than start parallel scans.
  ...
  private async runPending(): Promise<void> {
    ...
    // One scan owns this signal. Changes arriving during it remain pending for a following pass.
