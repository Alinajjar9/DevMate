// Adapt VS Code files and symbols to the indexer's input format.
// Read only eligible workspace files and recheck paths/content before using them.
...
async function readValidatedWorkspaceFile(
  ...
  // Reading yields control. Recheck the file and its parents instead of trusting the initial stat.
...
async function readDocumentSymbolRanges(
  ...
  try {
    ...
    assertNotCancelled(signal);
    // Symbol lookup may take time; use its ranges only if they still describe these exact bytes.
