// Start, authenticate, monitor, and stop the local backend process DevMate owns.
// Prefer the packaged executable and fall back to configured Python launchers.
...
import type { ValidatedCommand } from '../workspace/commandTools';
...
export function createBackendRequestToken(): string {
  // A new random token ties the listener to this managed launch, not merely to a port number.
...
export class LocalBackendManager {
  ...
  private runExclusive(forceRestart: boolean): Promise<boolean> {
    // Startup and restart requests share one operation so they cannot launch competing backends.
    if (this.disposed) {
  ...
  private scheduleRestart(): void {
    // Limit repeated crashes within a time window instead of restarting a broken backend forever.
