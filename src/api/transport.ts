...
// Bounded JSON and NDJSON transport, including timeout and cancellation cleanup.
...
/** Use fetch for a JSON request, distinguishing user cancellation from timeout and always removing listeners. */
...
/** Read a complete backend response through Node HTTP, with a byte limit and one cleanup path for every outcome. */
...
/**
 * Read newline-delimited JSON from the backend, forwarding live events and returning the final result.
 * Provider SSE is decoded by the backend; this connection expects NDJSON rather than data: lines.
 */
export async function requestStream(
  ...
  return new Promise((resolve) => {
    ...
    // Network chunks can split a UTF-8 character as well as a JSON line; keep both boundaries intact.
    ...
    try {
      ...
      backendRequest = requestFunction(endpoint, {
        ...
        response = incomingResponse;
        const statusCode = incomingResponse.statusCode ?? 0;
        // Only lack of streaming support permits the JSON fallback; other failures should keep their real cause.
        ...
        incomingResponse.on('end', () => {
          ...
          // The last JSON event may arrive without a trailing newline.
