...
test('requires the current structured command format', () => {
  assert.throws(
    () => parseRunCommandArguments({ command: 'python -m unittest test_app.py -v' }),
    /unsupported fields/
  );
  assert.throws(
    () => parseRunCommandArguments({ executable: 'python', args: '-m unittest' }),
    /args must be an array/
  );
  assert.throws(
    () => parseRunCommandArguments({ executable: 'python', arguments: ['-m', 'unittest'] }),
    /unsupported fields/
  );
});
