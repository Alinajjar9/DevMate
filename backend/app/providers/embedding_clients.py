# Request embeddings from Ollama or an OpenAI-compatible endpoint.
# Check remote-transfer consent and destination safety before sending source text.
...
class HttpEmbeddingProvider:
    ...
    async def embed(self, request: EmbeddingRequest) -> EmbeddingBatch:
        # Validate consent before DNS or HTTP: remote code transfer must be explicit.
        ...
        try:
            async with httpx.AsyncClient(
                ...
                # Do not forward source text or credentials to an unchecked redirect host.
...
def _read_openai_vectors(value: object, expected_count: int) -> list[object]:
    ...
    # Reconnect out-of-order provider rows to their original input chunks using the returned index.
...
def _normalize_vectors(
    ...
    for raw_vector in value:
        ...
        # Give each vector length one so later cosine comparisons are simple dot products.
