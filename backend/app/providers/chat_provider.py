# Call OpenAI-compatible chat providers and parse their completions/streams.
# Validate destination URLs and addresses before forwarding provider credentials.
...
from ..api.api_models import ProviderName, ReasoningEffort
...
class ChatStreamEvent:
    ...
    # Completion-only providers have no live deltas. Validate their full text before previewing it.
    deferred_preview: str | None = None
...
class StreamingChatProvider(ChatProvider, Protocol):
    def stream(self, request: ChatCompletionRequest) -> AsyncIterator[ChatStreamEvent]: ...
...
async def stream_chat_completion(
    ...
    """Use native streaming when available; adapt completion-only providers here."""
    ...
    yield ChatStreamEvent(
        ...
        deferred_preview=completion.content,
    )
