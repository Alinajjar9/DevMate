# Store normalized vectors separately from source chunks for each workspace.
# Configuration and content checks stop incompatible or stale vectors being reused.
...
class EmbeddingRepository:
    ...
    # Vectors from different models/dimensions cannot share one similarity ranking.
    ...
    def store_embeddings(
        ...
        try:
            with self._store.transaction() as connection:
                ...
                # The model request may have outlived an edit. Check the current chunk hash before saving.
    ...
    # Page by chunk ID so search does not need to load every vector into memory at once.
    def load_embeddings(
        self,
        workspace_key: str,
        configuration: EmbeddingConfiguration,
