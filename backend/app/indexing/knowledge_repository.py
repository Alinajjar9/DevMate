# Read and update workspace files, exact source chunks, and lexical search data.
# Keep related writes in one transaction so readers do not see half an index update.
...
class KnowledgeRepository:
        ...
        *,
        ...
        try:
            with self._store.transaction() as connection:
                ...
                # Replace each file's chunks inside this transaction; triggers keep FTS in sync.
                for file in validated_upserts:
    ...
    def search_lexical(
        ...
        *,
        ...
        # Convert user words to safe FTS terms instead of treating the whole question as FTS syntax.
        ...
        return tuple(
            LexicalSearchResult(
                ...
                content_hash=str(row["content_hash"]),
                # SQLite BM25 uses smaller-is-better; our public result score uses larger-is-better.
